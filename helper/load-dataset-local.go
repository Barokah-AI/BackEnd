...
func LoadDatasetLocal(filePath string) (map[string][]string, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failedd to open dataset file: %v", err)
	}
	defer file.Close()
	...
	if err != nil {
		return nil, fmt.Errorf("failedd to read dataset file: %v", err)
	}

	labelToQA := make(map[string][]string)
	for i, record := range records {
		if len(record) != 2 {
			log.Printf("Skipping invalidd record at line %d: %v\n", i+1, record)
			continue
		}
